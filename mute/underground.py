...
def _do_convolution (s_fluxes, survival_probability_tensor):
    ...
    assert s_fluxes.shape[0] == len(constants._ENERGIES), f"The surface flux matrix must use the default MUTE energy grid, which has length {len(constants._ENERGIES)}, not {s_fluxes.shape[0]}."
    ...
    widths = np.repeat(constants._E_WIDTHS, s_fluxes.shape[1]).reshape((len(constants._ENERGIES), s_fluxes.shape[1]))
...
def calc_u_fluxes (s_fluxes = None, survival_probability_tensor = None, full_tensor = False, primary_model = "GSF", interaction_model = "SIBYLL-2.3c", atmosphere = "CORSIKA", location = "USStd", month = None, output = None, file_name = "", force = False):
    
    """
    Calculate underground fluxes in units of [(cm^2 s sr MeV)^-1] for default surface energy grid and zenith angles.
    
    The default surface energy grid is given by constants._ENERGIES, and the default zenith angles are given by constants._ANGLES.
    ...
    if output is None: output = constants.get_output()
    ...
    if constants.get_verbose() > 1: print("Calculating underground fluxes.")
    
    if s_fluxes is None:
        
        s_fluxes = surface.load_s_fluxes_from_file(primary_model, interaction_model, atmosphere, location, month, force=force)
    
    if survival_probability_tensor is None:
        
        survival_probability_tensor = propagation.load_survival_probability_tensor_from_file(force=force)
    ...
    if s_fluxes is None or survival_probability_tensor is None:
        ...
        raise Exception("Underground fluxes not calculated. The surface flux matrix or survival probability tensor was not provided or loaded correctly.")
    
    assert s_fluxes.shape[0] == survival_probability_tensor.shape[2], f"The surface flux matrix and survival probability tensor must both use the default MUTE energy grid, which has length {len(constants._ENERGIES)}, not {s_fluxes.shape[0]} or {survival_probability_tensor.shape[2]}."
    ...
    if constants.get_overburden() == "flat" and not full_tensor:
        ...
        interp_u_fluxes_from_convolution = sciint.interp1d(constants._ANGLES_FOR_S_FLUXES, u_fluxes_from_convolution, axis = 2, kind = "linear", fill_value = 0)(constants._ANGLES)
        ...
        if output:
            ...
            constants._check_directory(os.path.join(constants.get_directory(), "underground"), force=force)
            ...
            for u in range(len(constants._ENERGIES)):
                ...
                for j in range(len(constants.angles)):
                    ...
                    file_out.write("{0:1.14f} {1:1.5f} {2:1.14e}\n".format(constants._ENERGIES[u], constants.angles[j], u_fluxes[u, j]))

            file_out.close()

            if constants.get_verbose() > 1: print(f"Underground fluxes written to {file_name}.")
        ...
        if output:
            ...
            for x in range(len(constants._SLANT_DEPTHS)):
                ...
                for u in range(len(constants._ENERGIES)):
                    ...
                    for j in range(len(constants._ANGLES_FOR_S_FLUXES)):
                        
                        file_out.write("{0:1.5f} {1:1.14f} {2:1.5f} {3:1.14e}\n".format(constants._SLANT_DEPTHS[x], constants._ENERGIES[u], constants._ANGLES_FOR_S_FLUXES[j], u_fluxes[x, u, j]))
...
def _create_interpolator (u_intensities):
    ...
    _interpolator = sciint.interp2d(constants._SLANT_DEPTHS, constants._ANGLES_FOR_S_FLUXES, np.log(u_intensities.T), kind = "linear", fill_value = -np.inf)
...
def calc_u_intensities (method, output = None, file_name = "", force = False, **kwargs):
    ...
    u_intensities : NumPy ndarray
        An array containing the underground intensities. For flat overburdens (if method is "sd", "eq", or "tr"), the length will be that of angles, and the intensities will be in units of [(cm^2 s sr)^-1]. For mountain overburdens (if method is "dd"), the shape will be (len(constants.mountain_zenith), len(constants.mountain_azimuthal)), and the intensities will be in units of [(cm^2 s sr km.w.e.)^-1].
    """
    ...
    E_th_i = np.argmin(np.abs(constants._ENERGIES - E_th))
    
    if E_th_i != 0 and constants.get_verbose() > 1:
        
        print("Setting energy threshold to {0:1.3f} MeV (the closest value in the energy grid to {1} MeV).".format(constants._ENERGIES[E_th_i], E_th))
    ...
    assert u_fluxes.shape == (28, 91, 20), f"The underground flux tensor does not have the correct shape. The shape must be (28, 91, 20), not {u_fluxes.shape}. Set full_tensor to True in the underground.calc_u_fluxes() function in order to return a tensor of shape (28, 91, 20). Alternatively, try passing a surface flux matrix and / or a survival probability tensor in as parameters."
    ...
    if method == "sd" or method == "eq":
        ...
        return _calc_u_intensities_sd(method, u_fluxes, angles, E_th_i, primary_model, interaction_model, atmosphere, location, month, output, file_name, force)
    
    # Calculate the true vertical underground intensities
    
    elif method == "tr":
        
        return _calc_u_intensities_tr(u_fluxes, depths, E_th_i, primary_model, interaction_model, atmosphere, location, month, output, file_name, force)
    
    # Calculate the double-differential underground intensities
    
    elif method == "dd":
        
        return _calc_u_intensities_dd(u_fluxes, E_th_i, primary_model, interaction_model, atmosphere, location, month, output, file_name, force)
        
    else:
        
        raise ValueError("\"{0}\" is not a valid method. Use \"sd\", \"tr\", \"eq\", or \"dd\". See the Tutorial at {1} for an explanation.".format(method, "https://github.com/wjwoodley/mute/blob/main/docs/Tutorial.md#calculating-underground-intensities"))

# Calculate single-differential underground intensities

def _calc_u_intensities_sd(method, u_fluxes, angles, E_th_i, primary_model, interaction_model, atmosphere, location, month, output, file_name, force):
    ...
    if constants.get_overburden() != "flat":
        ...
        raise ValueError("Single-differential underground intensities can only be calculated for flat overburdens. See the Tutorial at {0} for an explanation.".format("https://github.com/wjwoodley/mute/blob/main/docs/Tutorial.md#calculating-underground-intensities"))
    
    assert method in ["sd", "eq"]
    
    if u_fluxes is None:
        
        raise Exception("Underground intensities not calculated. The underground flux matrix was not calculated correctly.")
    
    # Calculate the underground intensities
    # Zeroth index = Slant depth
    # First index  = Zenith angle (from constants._ANGLES_FOR_S_FLUXES)
        
    u_intensities = np.zeros((len(constants._SLANT_DEPTHS), len(constants._ANGLES_FOR_S_FLUXES)))

    if constants.get_verbose() > 1: print("Calculating underground intensities.")
    ...
    for x in range(len(constants._SLANT_DEPTHS)):
        ...
        for j in range(len(constants._ANGLES_FOR_S_FLUXES)):
            ...
            u_intensities[x, j] = scii.simpson(u_fluxes[x, E_th_i:, j], constants._ENERGIES[E_th_i:])
    ...
    gd, ga = np.meshgrid(constants._SLANT_DEPTHS, constants._ANGLES_FOR_S_FLUXES)
    ad, aa = np.meshgrid(constants.slant_depths, constants.angles)

    u_intensities = np.nan_to_num(np.exp(sciint.griddata((gd.flatten(), ga.flatten()), np.log(u_intensities.T.flatten()), (ad, aa))))
    ...
    if not (len(angles) == len(constants.angles) and np.allclose(angles, constants.angles)):
    ...
    if constants.get_verbose() > 1: print("Finished calculating underground intensities.")
    ...
    if output:
        ...
        if file_name == "" or not isinstance(file_name, str):
            ...
            file_name = os.path.join(constants.get_directory(), "underground", "{0}_Underground_Intensities_{1}.txt".format(constants.get_lab(), method))
        ...
        if constants.get_verbose() > 1: print(f"Underground intensities written to {file_name}.")
    
    return u_intensities

# Calculate true vertical underground intensities

def _calc_u_intensities_tr(u_fluxes, depths, E_th_i, primary_model, interaction_model, atmosphere, location, month, output, file_name, force):
    ...
    if constants.get_overburden() != "flat":
        ...
        raise ValueError("True vertical underground intensities can only be calculated for flat overburdens. See the Tutorial at {0} for an explanation.".format("https://github.com/wjwoodley/mute/blob/main/docs/Tutorial.md#calculating-underground-intensities"))
    ...
    if constants.get_verbose() > 1: print("Calculating true vertical underground intensities.")
    
    u_fluxes_tr      = u_fluxes[:, :, 0]
    u_intensities_tr = [scii.simpson(u_fluxes_tr[x, E_th_i:], constants._ENERGIES[E_th_i:]) for x in range(len(constants._SLANT_DEPTHS))]

    if constants.get_verbose() > 1: print("Finished calculating true vertical underground intensities.")

    # Interpolate to the slant depths given by the set vertical depth or the user-defined slant depths

    u_intensities_tr = np.exp(np.interp(depths, constants._SLANT_DEPTHS, np.log(u_intensities_tr)))
...
def _calc_u_intensities_dd(u_fluxes, E_th_i, primary_model, interaction_model, atmosphere, location, month, output, file_name, force):
    ...
    if constants.get_overburden() != "mountain":
        ...
        raise ValueError("Double-differential underground intensities can only be calculated for non-flat overburdens. See the Tutorial at {0} for an explanation.".format("https://github.com/wjwoodley/mute/blob/main/docs/Tutorial.md#calculating-underground-intensities"))
    ...
    interpolation_result = np.hstack([np.exp(_interpolator(depth, angle)) for depth, angle in zip(constants._mountain_slants_all, constants._mountain_zenith_all)])

    # Calculate the intensities at the zenith and azimuthal angles specified in the mountain profile file

    interp_u_intensities = np.nan_to_num(np.reshape(interpolation_result, (len(constants.mountain_zenith), len(constants.mountain_azimuthal))))
    ...
    if output:
        ...
        if file_name == "" or not isinstance(file_name, str):
            ...
            file_name = os.path.join(constants.get_directory(), "underground", "{0}_Underground_Intensities_dd.txt".format(constants.get_lab()))

        file_out = open(file_name, "w")

        for j in range(len(constants.mountain_zenith)):

            for az in range(len(constants.mountain_azimuthal)):

                file_out.write("{0:1.5f} {1:1.5f} {2:1.14e}\n".format(constants.mountain_zenith[j], constants.mountain_azimuthal[az], interp_u_intensities[j, az]))
...
def calc_u_tot_flux (force = False, **kwargs):
    ...
    if constants.get_overburden() == "flat":
        ...
        if u_intensities is None:
            ...
            raise Exception("Total underground flux not calculated. The underground intensities were not calculated properly.")
    ...
    # Calculate the total underground flux for mountains
        ...
        u_tot_flux = scii.simpson([scii.simpson(u_intensities[j, :], np.radians(constants.mountain_azimuthal)) for j in range(len(constants.mountain_zenith))][::-1], np.cos(np.radians(constants.mountain_zenith[::-1])))
